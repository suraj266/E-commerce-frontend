 ...
 *
 * Contains all the stateful storefront UI for a PDP: image gallery, variant
 * picker, stock badges, price block, add-to-cart / buy-now, wishlist and the
 * sticky mobile buy bar. Server-fetched `product` data is passed in as a prop
 * (see the sibling `page.tsx` Server Component, which owns SEO metadata +
 * JSON-LD and calls `notFound()` for missing/inactive products).
 *
 * The JSX here is the original PDP markup, unchanged — only the data source
 * moved from a client `useQuery` to a server-provided prop.
 */
...
import { AlertTriangle, Box, ChevronRight, ShoppingCart, Zap } from "lucide-react";

import { parseSpecifications, Product } from "@/types/product.types";
